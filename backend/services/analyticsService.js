...
export const getHealthScore = async () => {
    ...
    for (const vehicle of vehicles) {
        ...
        const { data: maintenanceLogs } = await supabase.from('maintenance_logs').select('*').eq('vehicle_id', vehicle.id);
        const { data: tripLogs } = await supabase.from('trip_logs').select('*').eq('vehicle_id', vehicle.id);
        const { data: expenseLogs } = await supabase.from('expense_logs').select('*').eq('vehicle_id', vehicle.id);

        let totalFuelCost = 0;
        let totalQuantity = 0;
        fuelLogs?.forEach(log => {
            totalFuelCost += Number(log.total_cost);
            totalQuantity += Number(log.quantity);
        });

        let totalDistance = 0;
        let totalRevenue = 0;
        let totalDaysActive = new Set();
        tripLogs?.forEach(log => {
            totalDistance += Number(log.distance);
            totalRevenue += Number(log.revenue || 0);
            if (log.start_date) totalDaysActive.add(log.start_date.split('T')[0]);
        });
        ...
        expenseLogs?.forEach(log => {
            totalOtherExpenses += Number(log.amount);
        });
        ...
        // Fleet utilization: active days / 30 days (assuming monthly evaluation, or just return absolute active days)
...
export const getExpenseForecast = async () => {
    ...
    const { data: allMaintenance, error: e3 } = await supabase.from('maintenance_logs').select('service_date, cost');
    ...
    allMaintenance?.forEach(log => addToMonth(log.service_date, log.cost));
...
export const getFuelTheft = async () => {
    ...
    for (const vehicle of vehicles) {
        ...
        for (let i = 0; i < fuelLogs.length; i++) {
            ...
            const capacity = Number(vehicle.fuel_capacity);
            ...
            // 2. Far above average (e.g. > 50% of avg when we have > 3 logs)
...
export const getDashboard = async () => {
    ...
    fuelLogs?.forEach(l => totalFuelCost += Number(l.total_cost));
    expenseLogs?.forEach(l => totalExpense += Number(l.amount));
    maintLogs?.forEach(l => totalExpense += Number(l.cost));
